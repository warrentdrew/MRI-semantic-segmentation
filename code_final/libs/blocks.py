...
def conv1x1_selu(mode, filters, initializer, lbda, padding = 'same'):
    if mode == '2D':
        return lambda x : Activation('selu')(Conv2D(filters,
                                              kernel_size = (1, 1),
                                              strides= (1, 1),
                                              padding=padding,
                                              kernel_initializer = initializer,
        ...
        return lambda x: Activation('selu')(Conv3D(filters,
                                             kernel_size=(1, 1, 1),
                                             strides=(1, 1, 1),
                                             padding=padding,
                                             kernel_initializer=initializer,
...
def MR_block(mode, filters, initializer, lbda, padding='same'):
    # implementation of the merge-and-run block in https://arxiv.org/pdf/1611.07718.pdf
    def MR_instance(x,y):
        mid = Add()([x,y])
        x_conv = MR_local_path(mode, filters, initializer, lbda, padding)(x)
        y_conv = MR_local_path(mode, filters, initializer, lbda, padding)(y)
...
def resnext_branch(fin, fout, lbda, initializer = 'he_normal', padding = 'same'):
    def resnext_branch_instance(x):
        x = conv1x1_relu('3D', filters=fin, initializer=initializer, lbda=lbda, padding=padding)(x)
        x = bn_relu_conv_3x3('3D', filters=fin, initializer=initializer, lbda=lbda, padding= padding)(x)
        out = conv1x1_relu('3D', filters=fout, initializer=initializer, lbda=lbda, padding=padding)(x)
        return out
    return resnext_branch_instance


def resnext_blk(sp, fin, fout, lbda, initializer = 'he_normal', padding = 'same'):
    def resnext_blk_instance(x):
        input = conv1x1_relu('3D', filters=fout, initializer=initializer, lbda=lbda, padding=padding)(x)
        branch = [input]
        for i in range(sp):
            x = resnext_branch(fin, fout, lbda, initializer, padding)(input)
            branch.append(x)
        out = Add()(branch)
        return out
    return resnext_blk_instance

def resBlock(filters, lbda, initializer = 'he_normal', padding = 'same', scale = 0.1):
    def resblk_instance(x):
        res = Conv2D(filters,
                     kernel_size= (3,3),
                     strides= (1,1),
                     padding= padding,
        ...
        res = Conv2D(filters,
                     ...
                     padding= padding,
                     kernel_initializer= initializer,
                     kernel_regularizer = regularizers.l2(lbda))(res)
        res = Lambda(lambda x: x*scale)(res)
        return Add()([x, res])
    return resblk_instance
