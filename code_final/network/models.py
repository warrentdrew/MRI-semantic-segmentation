...
def DenseNet2D(in_shape, k, ls, theta, k_0, lbda=0, out_res=None, feed_pos=False, pos_noise_stdv=0):
    """Densely Connected Convolutional Neural Network for 2D patches."""
...
def DenseNet3D(in_shape, k, ls, theta, k_0, lbda=0, out_res=None, feed_pos=False, pos_noise_stdv=0):
    """Densely Connected Convolutional Neural Network for 3D crops."""
    in_ = Input(shape=in_shape, name='input_X')
...
class resize_2D(Layer):
    """Resize Layer to generate a smaller output in 2D."""
...
class resize_3D(Layer):
    """Resize Layer to generate a smaller output in 3D."""
    def __init__(self, out_res=24, **kwargs):
        self.input_dim = None
        self.out_res = out_res
...
# l layers within one block
# k growth rate
def denseBlock(mode, l, k, lbda):
    """Generate one DenseBlock with parameters l and k."""
    if mode == '2D':
