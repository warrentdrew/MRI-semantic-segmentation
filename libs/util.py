...
def findCh_dir(channel, lst_dir):
    """Find channel direction."""
    n = int(channel[-1])
    name = channel[:-1]
    ch_dir = list(filter(lambda dir_name: dir_name[-4]==dictChName[name], lst_dir))[n]
    return ch_dir
...
def check_patients(path, lst_ch, lst_cl, path_save=None, verbose=False):
    ...
    for patient in patients:
        ...
        for ch_index,ch in enumerate(lst_ch):
            try:
                ch_dir = findCh_dir(ch, lstChDirNames)
            except IndexError:
                if verbose:
        ...
        for cl_index, cl in enumerate(lst_cl):
            label_path = os.path.join(PathNii, lbl_switch * 'label_' + cl + '2d.nii')
            brk = False            
            try:
            ...
            try:
                ...
                brk = True
                break
            NiiPathsList[cl_index] = label_path
