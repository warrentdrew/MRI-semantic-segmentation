...
def crop3D(arr, pos, size):
    ...
    if (np.array(arr.shape[:3]) - end).min() >= 0:
        result = arr[pos[0]:end[0], pos[1]:end[1], pos[2]:end[2], :]  # delete the channel dim
    else:
        print('Invalid arguments, returning None.')
        result = None
    return result
...
def crop(dim, arr, pos, cropsize):
    ...
    if dim == 2:
        result = crop2D(arr, pos, cropsize)
    elif dim == 3:
        result = crop3D(arr, pos, cropsize)
    else:
        raise 'Unknown mode.'
    return result


def augmentation(dim, patient, label, samples_size, cropsize_X, cropsize_Y,
                 border):  # here data augmentation is just crop
    ...
    shape_X1 = (samples_size,) + dim * (cropsize_X,)  # so its a 32 patch size with 4 sample for each patient
    # [4,32,32,32]
    dist = (cropsize_X - cropsize_Y) // 2
    pos_x_Y = pos_x_X + dist  # pos_x_Y stands for the position of the labels, find the corresponding pixel position according to
    # the pixel pisition in imgs
    pos_y_Y = pos_y_X + dist
    if dim == 2:
        pos_z_Y = pos_z_X
    elif dim == 3:
        pos_z_Y = pos_z_X + dist
    else:
        raise "Invalid mode."
    shape_Y1 = (samples_size,) + dim * (cropsize_Y,)

    channels = patient.shape[-1]           #we have only one channel in our case
